...
function genPendingTx() {
  ...
  return Array.from({length:8}, (_,i) => ({
    ...
    user:      ["alice.kovacs@email.com","bob.tanaka@email.com","clara.osei@email.com","dave.muller@email.com"][i%4],
    type:      types[i%2],
    coin:      coins[i%5],
...
function Spark({ data, color, w=80, h=28 }) {
  ...
  const up = color || (data[data.length-1] >= data[0] ? "#a855f7" : "#ef4444");
  return (
    <svg width={w} height={h} style={{display:"block",overflow:"visible"}}>
      <polyline points={pts} fill="none" stroke={up} strokeWidth="1.5" strokeLinejoin="round"/>
    </svg>
  );
}
...
export default function VaultXCrypto() {
  ...
  // Start with EMPTY users list — no demo users
  const [users,        setUsers]        = useState([]);
  const [userHistory,  setUserHistory]  = useState({});   // per-user tx history
  ...
  const fmtP = (v) => { const c = v>=0?"#a855f7":"#ef4444"; return <span style={{color:c}}>{v>=0?"+":""}{fmt(v)}%</span>; };
  ...
  const showAlert = (text, type="error") => {
    setAlertMsg({text,type}); setTimeout(()=>setAlertMsg({text:"",type:""}),4000);
  };
  ...
  const getUserHistory = (email) => userHistory[email] || [];
    ...
    const fetchPrices = async () => {
      try {
        const results = await Promise.all(COINS.map(async c => {
          ...
          return [c.sym, Number(json?.data?.amount)];
        }));
        if (!active) return;
        setPrices(prev => {
          const next = {...prev};
          results.forEach(([sym, price]) => {
            ...
            next[sym] = { price, change, spark:[...old.spark.slice(1),price] };
          });
          return next;
        });
      } catch(e) { /* silent */ }
    ...
    const id = setInterval(() => {
      setPrices(prev => {
        ...
        COINS.forEach(c => {
          ...
          const newP = old.price*(1+(Math.random()-0.495)*0.002);
          next[c.sym] = { price:newP, change:+(old.change+(Math.random()-0.495)*0.05).toFixed(2), spark:[...old.spark.slice(1),newP] };
        });
        return next;
      });
    }, 2500);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (Math.abs(x) > (tickerRef.current?.scrollWidth||0)/2) x = 0;
    }, 30);
    return () => clearInterval(id);
  }, [view]);
  ...
  const doAdminLogin = () => {
    if (loginForm.admin!==ADMIN_USER.username || loginForm.adminPw!==ADMIN_USER.password) {
      showAlert("Invalid admin credentials"); return;
    }
    setIsAdmin(true); setView("admin"); setAdminTab("users");
  ...
  const doGoogleLogin = (credentialResponse) => {
    try {
      const profile = jwt_decode(credentialResponse?.credential);
      const email = profile?.email;
      if (!email) { showAlert("Google login failed"); return; }
    ...
    } catch(e) { showAlert("Google authentication failed"); }
  };
  ...
  const doTrade = () => {
    ...
    if (side==="buy") {
      ...
      if (idx!==-1) holdings[idx] = {...holdings[idx], qty:+(holdings[idx].qty+qty).toFixed(6)};
      else holdings.push({sym:coin, qty});
      const updated = {...user, balance:+(user.balance-amount).toFixed(2), portfolio:+(user.portfolio+amount).toFixed(2), holdings};
      updateCurrentUser(updated);
      addUserTx(user.email, { id:`TX${Date.now()}`, type:"Buy", symbol:coin, amount:qty, value:amount, fee:+(amount*0.001).toFixed(2), status:"Completed", date:new Date().toLocaleDateString() });
      toast$(`Bought ${qty} ${coin}`, "success");
    } else {
      const holding = holdings.find(h=>h.sym===coin);
      if (!holding||qty>holding.qty) { showAlert(`Insufficient ${coin}`); return; }
      const newHoldings = holdings.map(h=>h.sym===coin?{...h,qty:+(h.qty-qty).toFixed(6)}:h).filter(h=>h.qty>0);
      const updated = {...user, balance:+(user.balance+amount).toFixed(2), portfolio:Math.max(0,+(user.portfolio-amount).toFixed(2)), holdings:newHoldings};
      updateCurrentUser(updated);
      addUserTx(user.email, { id:`TX${Date.now()}`, type:"Sell", symbol:coin, amount:qty, value:amount, fee:+(amount*0.001).toFixed(2), status:"Completed", date:new Date().toLocaleDateString() });
  ...
  const doSend = () => {
    ...
    const price   = prices[coin]?.price||1;
    const qty     = +(amount/price).toFixed(6);
    const holding = user.holdings?.find(h=>h.sym===coin);
    if (!holding||qty>holding.qty) { showAlert(`Insufficient ${coin} balance`); return; }
    const newHoldings = (user.holdings||[]).map(h=>h.sym===coin?{...h,qty:+(h.qty-qty).toFixed(6)}:h).filter(h=>h.qty>0);
    const updated = {...user, portfolio:Math.max(0,+(user.portfolio-amount).toFixed(2)), holdings:newHoldings};
    updateCurrentUser(updated);
    addUserTx(user.email, { id:`TX${Date.now()}`, type:"Withdrawal", symbol:coin, amount:qty, value:amount, fee:1.2, status:"Completed", date:new Date().toLocaleDateString() });
    setSendForm({coin:"BTC",amount:"",address:""});
    toast$(`Sent ${qty} ${coin} to ${address.slice(0,8)}…`, "success");
  };

  // ── ADMIN HANDLERS ─────────────────────────────────────────────────────────
  const doAdminDeposit = () => {
    const target = adminDepositForm.user;
    ...
    const tx = {
      id:`DP${String(Date.now()).slice(-6)}`, user:target, type:"Deposit",
      coin:adminDepositForm.coin, amount:+(amount/(BASE_PRICES[adminDepositForm.coin]||1)).toFixed(6),
      usd:amount, fee:+(amount*0.001).toFixed(2), submitted:new Date().toLocaleString(), network:adminDepositForm.network,
    };
    setPending(prev=>[tx,...prev]);
    addUserTx(target, { id:tx.id, type:"Deposit", symbol:adminDepositForm.coin, amount:tx.amount, value:amount, fee:tx.fee, status:"Completed", date:new Date().toLocaleDateString() });
  ...
  const doAdminAddUser = () => {
    ...
    const newUser = {
      ...
      staking:  createStaking(0),
      joined: new Date().toLocaleDateString(),
      verified:true, status:"Active", tier,
    };
    setUsers(prev=>[newUser,...prev]);
    setAdminUserForm({name:"",email:"",password:"",tier:"Basic"});
    toast$("Client added: "+name, "success");
  };
  ...
  const doAdminCreateWithdrawRequest = () => {
    const target = withdrawRequestForm.user;
    const amount = Number(withdrawRequestForm.amount);
    if (!target||!amount||amount<=0) { showAlert("Select user and enter amount"); return; }
    const tx = {
      id:`WD${String(Date.now()).slice(-6)}`, user:target, type:"Withdrawal",
      coin:withdrawRequestForm.coin, amount:+(amount/(BASE_PRICES[withdrawRequestForm.coin]||1)).toFixed(6),
      usd:amount, fee:+(amount*0.0015).toFixed(2), submitted:new Date().toLocaleString(), network:withdrawRequestForm.network,
    };
    setPending(prev=>[tx,...prev]);
    setWithdrawRequestForm({user:"",coin:"BTC",amount:"",network:"ERC-20"});
    toast$("Withdrawal request created", "success");
  };

  const doAdminRequestFee = () => {
    const target = feeRequestForm.user;
    ...
    const feeTx = {
      ...
      user:target, amount, reason:feeRequestForm.reason||"Service fee",
      currency:feeRequestForm.currency, created:new Date().toLocaleString(), status:"Pending",
    };
  ...
  const C = {
    ...
    bg2:      "#0d0a1a",
    bg3:      "#120e22",
    card:     "rgba(138,43,226,.06)",
    border:   "rgba(138,43,226,.18)",
    border2:  "rgba(138,43,226,.08)",
    purple:   "#a855f7",
    purple2:  "#7c3aed",
    purple3:  "#c084fc",
    accent:   "#06b6d4",
  ...
  const S = {
    ...
    nav:  { background:"rgba(7,5,15,.96)", backdropFilter:"blur(20px)", borderBottom:`1px solid ${C.border}`, padding:"0 24px", height:60, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, boxShadow:`0 0 40px rgba(138,43,226,.12)` },
    ...
    btn: (v="primary") => ({
      ...
      background:
        v==="primary" ? `linear-gradient(135deg,${C.purple2},${C.purple})` :
        v==="danger"  ? "linear-gradient(135deg,#b91c1c,#dc2626)" :
        v==="success" ? `linear-gradient(135deg,#15803d,${C.green})` :
        v==="ghost"   ? `rgba(138,43,226,.12)` : `rgba(138,43,226,.12)`,
    ...
    <div style={S.ticker}>
      <div ref={tickerRef} style={{display:"inline-flex",gap:0,willChange:"transform"}}>
        {[...COINS,...COINS,...COINS].map((c,i)=>{
          const p = prices[c.sym]; const up = p?.change>=0;
          return (
            <span key={i} style={{padding:"0 22px",fontSize:12,fontFamily:"monospace",color:up?C.purple3:C.red}}>
              {c.sym}/USD &nbsp; ${p?.price<1?p?.price.toFixed(4):fmt(p?.price)} &nbsp;
              <span style={{opacity:.6}}>{up?"+":""}{fmt(p?.change)}%</span>
            </span>
          );
        })}
      </div>
  ...
  const TickerMini = () => (
    <div style={{display:"flex",gap:20,overflow:"hidden",maxWidth:380}}>
      {COINS.slice(0,4).map(c=>{
        const p=prices[c.sym]; const up=p?.change>=0;
        return <div key={c.sym} style={{fontSize:12,color:C.text3,whiteSpace:"nowrap"}}>
          <span style={{color:C.text,fontWeight:600}}>{c.sym}</span>{" "}
          <span style={{fontFamily:"monospace",color:up?C.purple3:C.red}}>${p?.price<1?p?.price.toFixed(4):fmt(p?.price)}</span>
        </div>;
      })}
    </div>
  );
  ...
  const Landing = () => (
    <div style={S.app}>
      ...
      <TickerBar/>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"60px 24px 60px"}}>
        {/* Hero */}
        <div style={{textAlign:"center",marginBottom:60}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(138,43,226,.1)",border:`1px solid ${C.border}`,borderRadius:20,padding:"4px 16px",fontSize:12,color:C.purple3,marginBottom:22}}>
            <span style={S.ldot}/> Live market data · Real-time trading
          </div>
          <h1 style={{fontSize:56,fontWeight:800,letterSpacing:"-2px",lineHeight:1.1,color:C.text,marginBottom:18}}>
          ...
          <p style={{color:C.text3,fontSize:16,maxWidth:480,margin:"0 auto 30px"}}>
            Real-time prices, portfolio analytics, staking rewards and instant transfers — all in one elite vault platform.
          </p>
        ...
        <div style={{...S.card,display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:0,marginBottom:36,padding:0,overflow:"hidden"}}>
          {[["$2.4T","Total Market Cap"],["$94B","24h Volume"],["23,400+","Active Users"],["0.05%","Avg Trading Fee"]].map(([v,l],i)=>(
            <div key={i} style={{padding:"20px 24px",borderRight:i<3?`1px solid ${C.border2}`:"none"}}>
              <div style={{fontSize:24,fontWeight:700,color:C.purple3}}>{v}</div>
        ...
        <div style={{...S.g4,marginBottom:36}}>
          {COINS.slice(0,4).map(coin=>{
            const p=prices[coin.sym]; const up=p.change>=0;
            return (
              <div key={coin.sym} style={{...S.card,position:"relative",overflow:"hidden",cursor:"pointer"}} onClick={()=>setView("register")}>
                ...
                <div style={S.rowsb}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{width:32,height:32,borderRadius:"50%",background:coin.bg,border:`1px solid ${coin.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:coin.color}}>{coin.sym.slice(0,3)}</div>
                    <div><div style={{fontWeight:600,fontSize:13,color:C.text}}>{coin.sym}</div><div style={{fontSize:11,color:C.text3}}>{coin.name}</div></div>
                  </div>
                  <span style={S.tag(up?"green":"red")}>{up?"+":""}{fmt(p.change)}%</span>
                </div>
                <div style={{marginTop:14}}>
                  <div style={{fontSize:20,fontWeight:700,color:C.text}}>${p.price<1?p.price.toFixed(4):fmt(p.price)}</div>
                  <div style={{marginTop:8}}><Spark data={p.spark} color={up?C.purple:"#ef4444"}/></div>
                </div>
              </div>
            );
          })}
        ...
        <div style={S.card}>
          <div style={{...S.rowsb,marginBottom:16}}>
            <span style={{fontSize:16,fontWeight:600,color:C.text}}>Live Market</span>
            <span style={{fontSize:11,color:C.text3}}><span style={S.ldot}/>Auto-refresh 2.5s</span>
          </div>
          <div style={{overflowX:"auto"}}>
            <table style={S.tbl}>
              <thead><tr>{["#","Asset","Price","24h Change","Market Cap","Volume","Sparkline",""].map((h,i)=><th key={i} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {COINS.map((coin,i)=>{
                  const p=prices[coin.sym]; const up=p.change>=0;
                  return (
                    <tr key={coin.sym} style={{cursor:"pointer"}}>
                      ...
                      <td style={S.td}><Spark data={p.spark} color={up?C.purple:"#ef4444"} w={80} h={28}/></td>
        ...
        <div style={{...S.g3,marginTop:36}}>
          {[
            ...
            <div key={i} style={{...S.scard,transition:"border-color .2s",cursor:"default"}}>
              <div style={{fontSize:28,marginBottom:10}}>{f.icon}</div>
              <div style={{fontWeight:600,color:C.text,marginBottom:6}}>{f.title}</div>
              <div style={{fontSize:12,color:C.text3,lineHeight:1.7}}>{f.desc}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            !loginForm.adminMode ? (
              <>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <input name="email" type="email" autoComplete="email" style={S.inp} placeholder="you@email.com" value={loginForm.email} onChange={e=>setLoginForm(f=>({...f,email:e.target.value}))}/>
                </div>
                <div style={{marginBottom:18}}>
                  <label style={S.label}>Password</label>
                  <input name="password" type="password" autoComplete="current-password" style={S.inp} placeholder="••••••••" value={loginForm.password} onChange={e=>setLoginForm(f=>({...f,password:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&doLogin()}/>
                </div>
                <button style={{...S.btn(),width:"100%",padding:"12px",justifyContent:"center"}} onClick={doLogin}>Sign In →</button>
                <div style={{textAlign:"center",fontSize:12,color:C.text3,marginTop:16}}>
                  No account? <span style={{color:C.purple3,cursor:"pointer"}} onClick={()=>setView("register")}>Create one</span>
                </div>
                <div style={{textAlign:"center",marginTop:10}}>
                  <span style={{fontSize:11,color:C.text3,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setLoginForm(f=>({...f,adminMode:true}))}>Admin access</span>
                </div>
              </>
            ) : (
              <>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <input name="admin" type="text" autoComplete="username" style={S.inp} placeholder="admin" value={loginForm.admin} onChange={e=>setLoginForm(f=>({...f,admin:e.target.value}))}/>
                </div>
                <div style={{marginBottom:18}}>
                  <label style={S.label}>Password</label>
                  <input name="adminPw" type="password" autoComplete="current-password" style={S.inp} placeholder="••••••••" value={loginForm.adminPw} onChange={e=>setLoginForm(f=>({...f,adminPw:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&doAdminLogin()}/>
                </div>
                <button style={{...S.btn(),width:"100%",padding:"12px",justifyContent:"center"}} onClick={doAdminLogin}>Admin Sign In →</button>
                <div style={{textAlign:"center",marginTop:14}}>
                  <span style={{fontSize:11,color:C.text3,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setLoginForm(f=>({...f,adminMode:false}))}>← Back to user login</span>
                </div>
              </>
            )
          ) : (
            <>
              ...
              {[["Full name","text","John Smith","name","name"],["Email","email","you@email.com","email","email"],["Password","password","Min 6 chars","password","new-password"],["Confirm password","password","Repeat password","confirm","new-password"]].map(([lbl,type,ph,key,ac])=>(
                <div key={key} style={{marginBottom:14}}>
                  ...
                  <input name={key} type={type} autoComplete={ac} style={S.inp} placeholder={ph} value={regForm[key]} onChange={e=>setRegForm(f=>({...f,[key]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&doRegister()}/>
                </div>
              ))}
              <button style={{...S.btn(),width:"100%",padding:"12px",justifyContent:"center"}} onClick={doRegister}>Create Account →</button>
              <div style={{textAlign:"center",fontSize:12,color:C.text3,marginTop:16}}>
                Already have an account? <span style={{color:C.purple3,cursor:"pointer"}} onClick={()=>setView("login")}>Sign in</span>
              </div>
            </>
          )}
        </div>
    ...
    return (
      <div style={{...S.app,display:"flex",flexDirection:"column",height:"100vh"}}>
        <nav style={S.nav}>
          ...
          <TickerMini/>
          <div style={S.row}>
            <div style={{fontSize:13,color:C.text2}}>👤 {user?.name}</div>
          ...
          <div style={S.sidebar}>
            {navItems.map(it=>(
              <button key={it.id} style={S.sitem(dashTab===it.id)} onClick={()=>setDashTab(it.id)}>
                <span style={{fontSize:14,color:dashTab===it.id?C.purple3:C.text3}}>{it.icon}</span>{it.label}
              </button>
            ))}
            <div style={{flex:1}}/>
            <div style={{padding:"14px",background:`rgba(138,43,226,.1)`,border:`1px solid ${C.border}`,borderRadius:10,marginTop:8}}>
              ...
              <div style={{fontSize:10,color:C.text3,marginTop:2}}>Balance: ${fmt(user?.balance||0)}</div>
            </div>
          </div>
      ...
      <div>
        <div style={{...S.rowsb,marginBottom:20,flexWrap:"wrap",gap:12}}>
          ...
          <div style={S.row}>
            ...
            <button style={S.btn()} onClick={()=>setModal("send")}>↗ Send</button>
          </div>
        </div>
        ...
        <div style={{...S.g4,marginBottom:20}}>
          {[
            ...
            {label:"Invested",         val:"$"+fmt(user?.portfolio||0),sub:"In crypto",             c:C.accent},
            {label:"Staking Rewards",  val:"$"+fmt(+(user?.portfolio||0)*0.005,2), sub:"This month",c:C.gold},
          ].map((s,i)=>(
        ...
        <div style={S.g2}>
          ...
          <div style={S.card}>
            ...
            {tradeForm.amount && Number(tradeForm.amount)>0 && (
              <div style={{...S.scard,marginBottom:12,fontSize:12,color:C.text2}}>
                ≈ {(Number(tradeForm.amount)/(prices[tradeForm.coin]?.price||1)).toFixed(6)} {tradeForm.coin} &nbsp;·&nbsp; Fee: ${(Number(tradeForm.amount)*0.001).toFixed(2)}
              </div>
            )}
            <button style={{...S.btn(tradeForm.side==="buy"?"success":"danger"),width:"100%",justifyContent:"center",padding:"11px"}} onClick={doTrade}>
          ...
          <div style={S.card}>
            ...
            {holdings.length===0 ? (
              <div style={{textAlign:"center",color:C.text3,padding:"30px 0",fontSize:13}}>No holdings yet.<br/>Make a trade or deposit to get started.</div>
            ) : (
              holdings.slice(0,6).map((h,i)=>{
                const coin = COINS.find(c=>c.sym===h.sym)||COINS[0];
                ...
                return (
                  <div key={h.sym} style={{...S.rowsb,padding:"9px 0",borderBottom:i<holdings.length-1?`1px solid ${C.border2}`:"none"}}>
                    <div style={S.row}>
                      <div style={{width:28,height:28,borderRadius:"50%",background:coin.bg,border:`1px solid ${coin.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:coin.color}}>{h.sym.slice(0,3)}</div>
                      <div><div style={{fontWeight:500,fontSize:13,color:C.text}}>{h.sym}</div><div style={{fontSize:11,color:C.text3}}>{h.qty.toFixed(6)}</div></div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontWeight:600,color:C.text}}>${fmt(val)}</div>
        ...
        <div style={{...S.card,marginTop:16}}>
          <div style={{...S.rowsb,marginBottom:14}}>
            <span style={{fontSize:15,fontWeight:600,color:C.text}}>Recent Activity</span>
            <button style={{...S.btn("ghost"),padding:"5px 12px",fontSize:12}} onClick={()=>setDashTab("history")}>View all →</button>
          </div>
          {getUserHistory(user?.email).length===0 ? (
            <div style={{textAlign:"center",color:C.text3,padding:"20px 0",fontSize:13}}>No transactions yet.</div>
          ) : (
            <table style={S.tbl}>
              <thead><tr>{["Type","Asset","Amount","Value","Status","Date"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {getUserHistory(user?.email).slice(0,5).map(tx=>(
                  <tr key={tx.id}>
  ...
  const DashMarkets = () => (
    <div>
      ...
      <div style={S.sub}><span style={S.ldot}/>Updating every 2.5 seconds</div>
      <div style={{...S.g4,marginBottom:20}}>
        {COINS.slice(0,4).map(coin=>{
          const p=prices[coin.sym]; const up=p.change>=0;
          return (
            <div key={coin.sym} style={S.card}>
              <div style={S.rowsb}><div style={{fontSize:13,fontWeight:600,color:C.text}}>{coin.sym}</div><span style={S.tag(up?"green":"red")}>{up?"+":""}{fmt(p.change)}%</span></div>
              <div style={{fontSize:20,fontWeight:700,color:C.text,margin:"8px 0"}}>${p.price<1?p.price.toFixed(4):fmt(p.price)}</div>
          ...
          <table style={S.tbl}>
            ...
            <tbody>
              {COINS.map((coin,i)=>{
                ...
                return (
                  <tr key={coin.sym}>
                    <td style={{...S.td,color:C.text3}}>{i+1}</td>
                    <td style={S.td}><div style={S.row}><div style={{width:24,height:24,borderRadius:"50%",background:coin.bg,border:`1px solid ${coin.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:coin.color}}>{coin.sym.slice(0,3)}</div><span style={{fontWeight:600,color:C.text}}>{coin.sym}</span></div></td>
                    <td style={{...S.td,fontFamily:"monospace",fontWeight:600,color:C.text}}>${p.price<1?p.price.toFixed(4):fmt(p.price)}</td>
                    <td style={S.td}>{fmtP(p.change)}</td>
                    <td style={{...S.td,fontFamily:"monospace",color:C.green}}>${(p.price*1.03)<1?(p.price*1.03).toFixed(4):fmt(p.price*1.03)}</td>
                    <td style={{...S.td,fontFamily:"monospace",color:C.red}}>${(p.price*0.97)<1?(p.price*0.97).toFixed(4):fmt(p.price*0.97)}</td>
  ...
  const DashWallet = () => {
    ...
    return (
      <div>
        ...
        <div style={S.sub}>Manage your balances, send and receive</div>
        <div style={S.g2}>
          <div style={S.card}>
            <div style={{fontSize:15,fontWeight:600,color:C.text,marginBottom:16}}>Send Crypto</div>
            <div style={{marginBottom:12}}>
              <label style={S.label}>Asset</label>
              <select name="coin" style={{...S.inp,cursor:"pointer"}} value={sendForm.coin} onChange={e=>setSendForm(f=>({...f,coin:e.target.value}))}>
                {COINS.map(c=><option key={c.sym} value={c.sym}>{c.sym} — ${prices[c.sym]?.price<1?prices[c.sym]?.price.toFixed(4):fmt(prices[c.sym]?.price)}</option>)}
              </select>
            </div>
            <div style={{marginBottom:12}}>
              <label style={S.label}>Recipient Address</label>
              <input name="address" style={S.inp} placeholder="0x…" autoComplete="off" value={sendForm.address} onChange={e=>setSendForm(f=>({...f,address:e.target.value}))}/>
            </div>
            <div style={{marginBottom:16}}>
              <label style={S.label}>Amount (USD)</label>
              <input name="amount" style={S.inp} type="number" inputMode="decimal" autoComplete="off" placeholder="0.00" value={sendForm.amount} onChange={e=>setSendForm(f=>({...f,amount:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&doSend()}/>
            </div>
            {sendForm.amount && <div style={{...S.scard,marginBottom:12,fontSize:12,color:C.text2}}>Network fee: ~$1.20 · Estimated arrival: 1–3 min</div>}
          ...
          <div style={S.card}>
            ...
            <div style={{background:`rgba(138,43,226,.06)`,border:`1px solid ${C.border}`,borderRadius:12,padding:24,textAlign:"center",marginBottom:14}}>
              ...
              <div style={{fontFamily:"monospace",fontSize:11,color:C.text2,wordBreak:"break-all",lineHeight:1.8}}>
              ...
              <div style={{marginTop:14,display:"flex",justifyContent:"center",gap:8}}>
                ...
                <button style={{...S.btn("ghost"),padding:"6px 14px",fontSize:12}} onClick={()=>setModal("deposit")}>+ Deposit</button>
              </div>
            </div>
        ...
        <div style={{...S.card,marginTop:16}}>
          ...
          {holdings.length===0 ? (
            <div style={{textAlign:"center",color:C.text3,padding:"20px 0",fontSize:13}}>
              Your wallet is empty. Make a deposit or trade to get started.
              <div style={{marginTop:12}}><button style={{...S.btn("success"),padding:"8px 20px"}} onClick={()=>setModal("deposit")}>+ Deposit</button></div>
            </div>
          ) : (
            <table style={S.tbl}>
              ...
              <tbody>
                {holdings.map(h=>{
                  const coin = COINS.find(c=>c.sym===h.sym)||COINS[0];
                  const p    = prices[h.sym]; const up = p?.change>=0;
                  return (
                    <tr key={h.sym}>
                      <td style={S.td}><div style={S.row}><div style={{width:24,height:24,borderRadius:"50%",background:coin.bg,border:`1px solid ${coin.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:coin.color}}>{h.sym.slice(0,3)}</div><span style={{fontWeight:600,color:C.text}}>{coin.name}</span></div></td>
                      <td style={{...S.td,fontFamily:"monospace"}}>{h.qty.toFixed(6)} {h.sym}</td>
                      <td style={{...S.td,fontFamily:"monospace",fontWeight:600}}>${fmt(h.qty*(p?.price||0))}</td>
                      <td style={S.td}>{fmtP(p?.change||0)}</td>
                      <td style={S.td}><div style={S.row}><button style={{...S.btn("success"),padding:"4px 11px",fontSize:11}} onClick={()=>{setTradeForm(f=>({...f,coin:h.sym,side:"buy"}));setDashTab("overview");}}>Buy</button><button style={{...S.btn("ghost"),padding:"4px 11px",fontSize:11}} onClick={()=>{setSendForm(f=>({...f,coin:h.sym}));setDashTab("wallet");}}>Send</button></div></td>
                    </tr>
                  );
                })}
              </tbody>
  ...
  const DashPortfolio = () => {
    ...
    const coin = sym => COINS.find(c=>c.sym===sym)||COINS[0];
    return (
      <div>
        ...
        <div style={S.g2}>
          <div style={S.card}>
            ...
            <div style={{fontSize:12,color:C.purple3,marginBottom:20}}>Available: ${fmt(user?.balance||0)}</div>
            {allocs.length===0 ? (
              <div style={{color:C.text3,fontSize:13,padding:"10px 0"}}>No holdings to display.</div>
            ) : allocs.map(a=>{
              ...
              return (
                <div key={a.sym} style={{marginBottom:14}}>
                  <div style={{...S.rowsb,marginBottom:5}}>
                    <span style={{fontSize:12,color:C.text2}}>{a.sym}</span>
                    <span style={{fontSize:12,fontWeight:600,color:C.text}}>{pct.toFixed(1)}% · ${fmt(a.val)}</span>
                  </div>
                  <div style={{height:5,background:`rgba(138,43,226,.1)`,borderRadius:3,overflow:"hidden"}}>
                    <div style={{height:"100%",width:pct+"%",background:`linear-gradient(90deg,${c.color}80,${c.color})`,borderRadius:3}}/>
                  </div>
                </div>
              );
            })}
          ...
          <div style={S.card}>
            ...
            {[["Today","$+0.00","0.0%","green"],["This Week","$+0.00","0.0%","green"],["This Month","$+0.00","0.0%","green"],["All Time","$+0.00","0.0%","green"]].map(([p,v,c,col])=>(
              <div key={p} style={{...S.rowsb,padding:"10px 0",borderBottom:`1px solid ${C.border2}`}}>
                <span style={{color:C.text3,fontSize:13}}>{p}</span>
                <div style={{textAlign:"right"}}>
                  <div style={{fontWeight:600,color:C.text,fontSize:13}}>{v}</div>
                  <div style={{fontSize:11,color:col==="green"?C.green:C.red}}>{c}</div>
                </div>
              </div>
            ))}
            <div style={{marginTop:16,fontSize:12,color:C.text3}}>Performance tracking is based on your trading activity.</div>
          </div>
        </div>
      </div>
    );
            ...
            return (
              <div key={i} style={{...S.card,position:"relative",overflow:"hidden"}}>
                ...
                <div style={{...S.rowsb,marginBottom:12}}>
                  <div style={S.row}>
                    <div style={{width:30,height:30,borderRadius:"50%",background:s.bg,border:`1px solid ${s.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:s.color}}>{s.sym.slice(0,3)}</div>
                    <div><div style={{fontWeight:600,color:C.text}}>{s.sym}</div><div style={{fontSize:11,color:C.text3}}>{s.name}</div></div>
                  </div>
                  <span style={S.tag("green")}>{s.apy}% APY</span>
                </div>
                <div style={S.g2}>
                  <div><div style={{fontSize:10,color:C.text3,marginBottom:3}}>STAKED</div><div style={{fontWeight:600,color:C.text}}>{staked.toFixed(4)} {s.sym}</div></div>
                  <div><div style={{fontSize:10,color:C.text3,marginBottom:3}}>MONTHLY</div><div style={{fontWeight:600,color:C.green}}>${fmt(monthly)}</div></div>
                </div>
                <div style={{marginTop:12}}>
                  <button style={{...S.btn(staked>0?"ghost":"success"),width:"100%",justifyContent:"center",padding:8,fontSize:12}}
                    onClick={()=>{
                      if(staked<=0){setTradeForm(f=>({...f,coin:s.sym,side:"buy"}));setDashTab("overview");toast$("Buy "+s.sym+" first to stake","info");}
                      else toast$("Staking increased for "+s.sym+"!","success");
                    }}>
  ...
  // History
  const DashHistory = () => {
    const txs = getUserHistory(user?.email);
    return (
      <div>
        ...
        <div style={S.card}>
          {txs.length===0 ? (
            <div style={{textAlign:"center",color:C.text3,padding:"30px 0",fontSize:13}}>No transactions yet. Start trading to see your history here.</div>
          ) : (
            <div style={{overflowX:"auto"}}>
              <table style={S.tbl}>
    ...
    return (
      <div style={{...S.app,display:"flex",flexDirection:"column",height:"100vh"}}>
        <nav style={{...S.nav,borderBottom:"1px solid rgba(239,68,68,.25)"}}>
          <div style={S.logo}>
            <div style={{...S.logoMark,background:"linear-gradient(135deg,#b91c1c,#dc2626)"}}>AD</div>
  ...
  const AdminUsers = () => (
    <div>
      <div style={S.rowsb}>
        ...
        <button style={S.btn()} onClick={()=>toast$("CSV exported","success")}>Export CSV</button>
      </div>
      ...
      <div style={{...S.g4,marginBottom:20}}>
        {[
          ...
          {l:"Total Deposits", v:"$"+fmt(users.reduce((a,u)=>a+u.balance,0))},
        ].map((s,i)=>(
          <div key={i} style={S.scard}>
            <div style={{fontSize:10,color:C.text3,marginBottom:4,textTransform:"uppercase",letterSpacing:".06em"}}>{s.l}</div>
      ...
      <div style={{...S.card,marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
          <div>
            ...
            <input style={S.inp} value={adminUserForm.name} onChange={e=>setAdminUserForm(f=>({...f,name:e.target.value}))} placeholder="Jane Doe"/>
          </div>
          <div>
            <label style={S.label}>Email</label>
            <input style={S.inp} type="email" value={adminUserForm.email} onChange={e=>setAdminUserForm(f=>({...f,email:e.target.value}))} placeholder="jane@email.com"/>
          </div>
          <div>
            <label style={S.label}>Password</label>
            <input style={S.inp} type="password" value={adminUserForm.password} onChange={e=>setAdminUserForm(f=>({...f,password:e.target.value}))} placeholder="••••••"/>
          </div>
          <div>
            <label style={S.label}>Tier</label>
            <select style={S.inp} value={adminUserForm.tier} onChange={e=>setAdminUserForm(f=>({...f,tier:e.target.value}))}>
      ...
      <div style={S.card}>
        {users.length===0 ? (
          <div style={{textAlign:"center",color:C.text3,padding:"30px 0"}}>No clients yet. Add one above.</div>
        ) : (
          <div style={{overflowX:"auto"}}>
            <table style={S.tbl}>
              ...
              <tbody>
                {users.map(u=>(
                  <tr key={u.id}>
                    ...
                    <td style={{...S.td,fontWeight:600,color:C.text}}>{u.name}</td>
                    ...
                    <td style={S.td}><span style={S.tag(u.tier==="Elite"?"yellow":u.tier==="Pro"?"purple":"")}>{u.tier}</span></td>
                    ...
                    <td style={S.td}>
                      <div style={S.row}>
                        <button style={{...S.btn("success"),padding:"3px 10px",fontSize:11}} onClick={()=>setModal({type:"fundUser",user:u})}>+ Fund</button>
  ...
  const AdminPending = () => (
    <div>
      ...
      <div style={S.sub}>All transactions awaiting review</div>
      <div style={{...S.g3,marginBottom:20}}>
        {[
          {l:"Total Pending",  v:pending.length,             c:"yellow"},
          {l:"Withdrawals",    v:pendingWithdrawals.length,  c:"red"},
          {l:"Deposits",       v:pendingDeposits.length,     c:"green"},
        ].map((s,i)=>(
          <div key={i} style={{...S.scard,borderLeft:`3px solid ${s.c==="yellow"?C.gold:s.c==="red"?C.red:C.green}`}}>
            <div style={{fontSize:10,color:C.text3,marginBottom:4,textTransform:"uppercase"}}>{s.l}</div>
      ...
      <div style={S.card}>
        <div style={{overflowX:"auto"}}>
          <table style={S.tbl}>
            <thead><tr>{["TX ID","User","Type","Coin","Amount","USD","Fee","Network","Submitted","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>
              {pending.map(tx=>(
                <tr key={tx.id}>
                  <td style={{...S.td,fontFamily:"monospace",color:C.text3,fontSize:11}}>{tx.id}</td>
                  ...
                  <td style={S.td}><div style={S.row}><button style={{...S.btn("success"),padding:"3px 10px",fontSize:11}} onClick={()=>removePendingTx(tx.id,"TX approved")}>Approve</button><button style={{...S.btn("danger"),padding:"3px 10px",fontSize:11}} onClick={()=>removePendingTx(tx.id,"TX rejected")}>Reject</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{...S.card,marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
          <div>
            ...
            <select style={S.inp} value={withdrawRequestForm.user} onChange={e=>setWithdrawRequestForm(f=>({...f,user:e.target.value}))}>
          ...
          <div>
            ...
            <select style={S.inp} value={withdrawRequestForm.coin} onChange={e=>setWithdrawRequestForm(f=>({...f,coin:e.target.value}))}>
              {COINS.map(c=><option key={c.sym} value={c.sym}>{c.sym}</option>)}
            </select>
          </div>
          <div>
            <label style={S.label}>Network</label>
            <select style={S.inp} value={withdrawRequestForm.network} onChange={e=>setWithdrawRequestForm(f=>({...f,network:e.target.value}))}>
              {["ERC-20","BEP-20","TRC-20","Native"].map(n=><option key={n} value={n}>{n}</option>)}
            </select>
          </div>
          <div>
            <label style={S.label}>Amount (USD)</label>
            <input type="number" min="0" style={S.inp} value={withdrawRequestForm.amount} onChange={e=>setWithdrawRequestForm(f=>({...f,amount:e.target.value}))} placeholder="1000"/>
          </div>
        </div>
        <button style={{...S.btn("success"),padding:"10px 20px"}} onClick={doAdminCreateWithdrawRequest}>Create Request</button>
      </div>
      <div style={S.card}>
        <table style={S.tbl}>
          <thead><tr>{["TX ID","User","Coin","Amount","USD","Fee","Network","Submitted","Action"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
          <tbody>
            {pendingWithdrawals.length===0 ? (
              <tr><td colSpan={9} style={{...S.td,textAlign:"center",color:C.text3}}>No pending withdrawals.</td></tr>
  ...
  const AdminDeposits = () => (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.2fr 0.8fr",gap:20,marginBottom:20}}>
        <div style={S.card}>
          ...
          <div style={{display:"grid",gap:12}}>
            <div>
              <label style={S.label}>Client</label>
              <select style={S.inp} value={adminDepositForm.user} onChange={e=>setAdminDepositForm(f=>({...f,user:e.target.value}))}>
                <option value="">Select client</option>
            ...
            <div>
              <label style={S.label}>Coin</label>
              <select style={S.inp} value={adminDepositForm.coin} onChange={e=>setAdminDepositForm(f=>({...f,coin:e.target.value}))}>
                {COINS.map(c=><option key={c.sym} value={c.sym}>{c.sym}</option>)}
              </select>
            </div>
            <div>
              <label style={S.label}>Network</label>
              <select style={S.inp} value={adminDepositForm.network} onChange={e=>setAdminDepositForm(f=>({...f,network:e.target.value}))}>
                {["ERC-20","BEP-20","TRC-20","Native"].map(n=><option key={n} value={n}>{n}</option>)}
              </select>
            </div>
            <div>
              <label style={S.label}>Amount (USD)</label>
              <input type="number" min="0" style={S.inp} placeholder="1000.00" value={adminDepositForm.amount} onChange={e=>setAdminDepositForm(f=>({...f,amount:e.target.value}))}/>
            </div>
            ...
            <div style={{fontSize:12,color:C.text3}}>Credits the selected client's balance and logs a deposit transaction in their history.</div>
          </div>
        </div>
        <div style={S.card}>
          ...
          <div style={{display:"grid",gap:10}}>
            {[
              ...
              {label:"Pending Deps",   value:pendingDeposits.length},
      ...
      <div style={S.card}>
        <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:14}}>Recent Deposit Log</div>
        <div style={{overflowX:"auto"}}>
          <table style={S.tbl}>
            <thead><tr>{["TX ID","User","Coin","Amount","USD","Fee","Network","Submitted","Action"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>
              {pendingDeposits.length===0 ? (
                <tr><td colSpan={9} style={{...S.td,textAlign:"center",color:C.text3}}>No pending deposits.</td></tr>
                ...
                <tr key={tx.id}>
                  ...
                  <td style={{...S.td,fontFamily:"monospace"}}>{tx.amount}</td>
                  <td style={{...S.td,fontFamily:"monospace"}}>${fmt(tx.usd)}</td>
                  <td style={{...S.td,fontFamily:"monospace",color:C.green}}>${fmt(tx.fee)}</td>
                  <td style={{...S.td,fontSize:11}}>{tx.network}</td>
                  <td style={{...S.td,fontSize:11,color:C.text3}}>{tx.submitted}</td>
                  <td style={S.td}><div style={S.row}><button style={{...S.btn("success"),padding:"3px 10px",fontSize:11}} onClick={()=>removePendingTx(tx.id,"Deposit confirmed")}>Confirm</button><button style={{...S.btn("danger"),padding:"3px 10px",fontSize:11}} onClick={()=>removePendingTx(tx.id,"Deposit removed")}>Remove</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  const AdminFees = () => {
    const platformRev = pending.reduce((a,b)=>a+b.fee,0)*0.6;
    return (
      <div>
        <div style={{...S.rowsb,marginBottom:20}}>
          <div><div style={S.hd}>Fee Collection</div><div style={S.sub}>Issue and manage fee requests</div></div>
          <button style={S.btn("success")} onClick={()=>toast$("Dashboard refreshed","success")}>Refresh</button>
        </div>
        <div style={{...S.g4,marginBottom:20}}>
          {[
            {l:"Active Clients",   v:users.length,          c:C.purple3, icon:"👥"},
            {l:"Pending Requests", v:feeRequests.length,    c:C.green,   icon:"⏳"},
            {l:"Collected Fees",   v:"$"+fmt(platformRev),  c:C.gold,    icon:"💰"},
            {l:"Coinbase Sync",    v:"Live",                 c:C.accent,  icon:"🔗"},
          ].map((s,i)=>(
            <div key={i} style={{...S.card,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{...S.card,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
            <div>
              <label style={S.label}>Client</label>
              <select style={S.inp} value={feeRequestForm.user} onChange={e=>setFeeRequestForm(f=>({...f,user:e.target.value}))}>
                <option value="">Select client</option>
                {users.map(u=><option key={u.email} value={u.email}>{u.name}</option>)}
              </select>
            </div>
            <div>
              ...
              <input type="number" min="1" style={S.inp} value={feeRequestForm.amount} onChange={e=>setFeeRequestForm(f=>({...f,amount:e.target.value}))} placeholder="100"/>
            </div>
            <div>
              <label style={S.label}>Currency</label>
              <select style={S.inp} value={feeRequestForm.currency} onChange={e=>setFeeRequestForm(f=>({...f,currency:e.target.value}))}>
                {["USD","BTC","ETH"].map(c=><option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={S.label}>Reason</label>
              <input style={S.inp} value={feeRequestForm.reason} onChange={e=>setFeeRequestForm(f=>({...f,reason:e.target.value}))} placeholder="Maintenance fee"/>
            </div>
          </div>
          <button style={{...S.btn("success"),padding:"10px 20px"}} onClick={doAdminRequestFee}>Send Fee Request</button>
        </div>
        <div style={S.card}>
          <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:14}}>Outstanding Requests</div>
          <div style={{overflowX:"auto"}}>
            <table style={S.tbl}>
              ...
              <tbody>
                {feeRequests.length===0 ? (
                  <tr><td colSpan={8} style={{...S.td,textAlign:"center",color:C.text3}}>No fee requests yet.</td></tr>
                ) : feeRequests.map(req=>(
                  <tr key={req.id}>
                    <td style={{...S.td,fontFamily:"monospace",color:C.text3,fontSize:11}}>{req.id}</td>
                    ...
                    <td style={{...S.td,color:C.text3}}>{req.created}</td>
            ...
            {COINS.map(coin=>{
              ...
              return (
                <tr key={coin.sym}>
                  ...
                  <td style={{...S.td,fontFamily:"monospace",fontWeight:600,color:C.text}}>${p.price<1?p.price.toFixed(4):fmt(p.price)}</td>
                  <td style={S.td}>{fmtP(p.change)}</td>
                  <td style={{...S.td,fontFamily:"monospace",color:C.green}}>${(p.price*0.999)<1?(p.price*0.999).toFixed(4):fmt(p.price*0.999)}</td>
                  <td style={{...S.td,fontFamily:"monospace",color:C.red}}>${(p.price*1.001)<1?(p.price*1.001).toFixed(4):fmt(p.price*1.001)}</td>
                  <td style={{...S.td,fontFamily:"monospace"}}>${fmt(p.price*21000/1000,1)}K</td>
  ...
  const AdminSettings = () => (
    <div>
      ...
      <div style={S.sub}>Configure VaultXcrypto parameters</div>
      <div style={S.g2}>
        {[
          {title:"Trading", fields:[["Trading Fee (%)","0.10"],["Min Trade ($)","10"],["Max Trade ($)","100,000"],["Daily Withdrawal ($)","50,000"]]},
          {title:"Security", fields:[["2FA Required","Enabled"],["KYC Level","Level 2"],["Session Timeout (min)","30"],["IP Whitelist","Disabled"]]},
          {title:"Email", fields:[["SMTP Host","smtp.sendgrid.net"],["Sender","noreply@vaultx.io"],["Verification","Enabled"],["Deposit Alerts","Enabled"]]},
          {title:"Network Fees", fields:[["BTC","0.0005 BTC"],["ETH Gas","Auto"],["BNB","0.0005 BNB"],["SOL","0.000025 SOL"]]},
        ].map((s,i)=>(
          <div key={i} style={S.card}>
            ...
            <button style={{...S.btn("success"),marginTop:6,padding:"8px 18px"}} onClick={()=>toast$("Settings saved!","success")}>Save</button>
          </div>
        ))}
      </div>
    </div>
        ...
        <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{background:`rgba(138,43,226,.08)`,border:`1px solid ${C.border}`,borderRadius:12,padding:18,marginBottom:18}}>
            ...
            <div style={{fontSize:12,color:C.text2,lineHeight:1.7}}>
              For security compliance, deposit wallets are generated by our team per-transaction. Contact support with your desired coin and amount — we'll send you a wallet address within minutes.
            </div>
          </div>
          <div style={{fontSize:12,color:C.text3,marginBottom:16}}>📧 support@vaultxcrypto.io</div>
          <div style={{display:"flex",gap:10}}>
            <button style={{...S.btn("success"),flex:1,justifyContent:"center"}} onClick={()=>{window.open("mailto:support@vaultxcrypto.io?subject=Deposit%20Request","_blank");toast$("Opening support email…","info");}}>Contact Support</button>
      ...
      <div style={S.modal} onClick={close}>
        <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{...S.row,gap:10,marginTop:16}}>
            ...
            <button style={{...S.btn("success"),flex:1,justifyContent:"center"}} onClick={()=>{doSend();if(!alertMsg.text)close();}}>Send →</button>
          </div>
        </div>
      </div>
    );
        ...
        <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{...S.scard,marginBottom:14}}>
            {[["Name",modal.user?.name],["Email",modal.user?.email],["Balance","$"+fmt(modal.user?.balance)],["Portfolio","$"+fmt(modal.user?.portfolio)],["Tier",modal.user?.tier],["Status",modal.user?.status],["Joined",modal.user?.joined]].map(([l,v])=>(
              <div key={l} style={{...S.rowsb,padding:"7px 0",borderBottom:`1px solid ${C.border2}`}}>
                <span style={{color:C.text3,fontSize:12}}>{l}</span>
                <span style={{color:C.text,fontWeight:500,fontSize:13}}>{v}</span>
              </div>
            ))}
          </div>
          <div style={{...S.g2,marginBottom:10}}>
            <button style={{...S.btn("success"),justifyContent:"center",padding:9}} onClick={()=>{setModal({type:"fundUser",user:modal.user});}}>+ Fund Account</button>
            <button style={{...S.btn("danger"),justifyContent:"center",padding:9}} onClick={()=>{toast$("Account suspended","info");close();}}>Suspend</button>
          </div>
      ...
      return (
        <div style={S.modal} onClick={close}>
          <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{marginBottom:12}}>
              <label style={S.label}>Coin</label>
              <select style={S.inp} value={fundCoin} onChange={e=>setFundCoin(e.target.value)}>
                {COINS.map(c=><option key={c.sym} value={c.sym}>{c.sym}</option>)}
              </select>
            </div>
            <div style={{marginBottom:18}}>
              <label style={S.label}>Amount (USD)</label>
              <input type="number" style={S.inp} value={fundAmount} onChange={e=>setFundAmount(e.target.value)} placeholder="1000.00" autoFocus/>
            </div>
            <div style={{...S.row,gap:10}}>
              <button style={{...S.btn("ghost"),flex:1,justifyContent:"center"}} onClick={close}>Cancel</button>
              <button style={{...S.btn("success"),flex:1,justifyContent:"center"}} onClick={()=>{
                const amount = Number(fundAmount);
                if (!amount||amount<=0) { toast$("Enter a valid amount","info"); return; }
                ...
                addUserTx(modal.user.email, {id:`DP${Date.now()}`,type:"Deposit",symbol:fundCoin,amount:+(amount/(BASE_PRICES[fundCoin]||1)).toFixed(6),value:amount,fee:+(amount*0.001).toFixed(2),status:"Completed",date:new Date().toLocaleDateString()});
                toast$(`$${fmt(amount)} credited to ${modal.user.name}`,"success");
                close();
              }}>Credit Funds</button>
            </div>
          </div>
