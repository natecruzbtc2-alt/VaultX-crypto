...
export const Spark = memo(({data,color,w=80,h=28})=>{
  ...
  const c=color||(data[data.length-1]>=data[0]?"#a855f7":"#ef4444");
  return(
    ...
    <div style={S.ticker}>
      <div ref={ref} style={{display:"inline-flex",willChange:"transform"}}>
        {[...COINS,...COINS,...COINS].map((c,i)=>{
          ...
          return(
            <span key={i} style={{padding:"0 24px",fontSize:12,fontFamily:"monospace",color:up?C.purple3:C.red}}>
              {c.sym}/USD &nbsp;
      ...
      {COINS.slice(0,4).map(c=>{
        ...
        return(
          <div key={c.sym} style={{fontSize:12,color:C.text3,whiteSpace:"nowrap"}}>
            ...
            <span style={{fontFamily:"monospace",color:up?C.purple3:C.red}}>
